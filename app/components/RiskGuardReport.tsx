...
export function RiskGuardReport() {
  return (
    <div className="border border-[var(--color-slate-line)] bg-[var(--color-slate)] text-[var(--color-slate-ink)] rounded-md overflow-hidden shadow-[0_24px_60px_-24px_rgba(13,17,23,0.6)]">
      ...
      <div className="grid sm:grid-cols-3 border-b border-[var(--color-slate-line)]">
      ...
      <div className="border-t border-[var(--color-slate-line)]">
        <div className="px-4 sm:px-6 py-2 flex items-center justify-between border-b border-[var(--color-slate-line)] bg-[var(--color-slate-2)]">
        ...
        {ROWS.map((r) => (
          <div
            key={r.n}
            className="px-4 sm:px-6 py-3 flex items-start sm:items-center justify-between gap-4 border-b border-[var(--color-slate-line)] last:border-b-0"
          >
            <div className="flex min-w-0 items-start sm:items-center gap-3 sm:gap-4">
              ...
              <span className="text-sm leading-snug text-[var(--color-slate-ink)]">{r.label}</span>
            </div>
            <span className="inline-flex shrink-0 items-center gap-2 font-mono text-[11px]">
      ...
      <div className="px-4 sm:px-6 py-3 flex flex-wrap items-center justify-between gap-2 bg-[var(--color-slate-2)] border-t border-[var(--color-slate-line)]">
...
function Cell({ label, value, last }: { label: string; value: string; last?: boolean }) {
  return (
    <div className={`px-4 sm:px-5 py-3 border-b sm:border-b-0 border-[var(--color-slate-line)] ${last ? "border-b-0" : "sm:border-r"}`}>
